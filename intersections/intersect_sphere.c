...
/*   Updated: 2026/01/13 19:16:42 by mkeerewe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	sphere_det(double a, double b, t_tuple s_to_r)
{
	...
	c = dot_product(s_to_r, s_to_r) - 1.0;
...
int	intersect_sphere(t_shape *sphere, t_ray ray, t_intersection res[2])
{
	...
	b = 2 * dot_product(ray_o.dir, s_to_r);
	discr = sphere_det(dot_product(ray_o.dir, ray_o.dir), b, s_to_r);
	if (discr < 0)
		return (0);
	if (equal(discr, 0.0))
		num = 1;
		...
		num = 2;
	res[0].shape = sphere;
	res[0].t = ((b * -1.0) - sqrt(discr)) / (2.0 * dot_product(ray_o.dir, ray_o.dir));
	if (num == 2)
	{
		res[1].shape = sphere;
		res[1].t = ((b * -1.0) + sqrt(discr)) / (2.0 * dot_product(ray_o.dir, ray_o.dir));
	}
	return (num);
}
